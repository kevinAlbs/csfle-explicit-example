...
func main() {
	...
	{
		fmt.Printf("Get without decryption ... begin\n")
		...
		unencryptedColl := unencryptedClient.Database("db").Collection("coll")
		res := unencryptedColl.FindOne(context.TODO(), bson.D{})
		if res.Err() != nil {
			log.Panicf("FindOne error: %v\n", res.Err())
		}
		var decoded bson.Raw
		...
		fmt.Printf("Document is stored in database as: %v\n", decoded)
		fmt.Printf("Get without decryption ... end\n")
	}
}
